...
- `myx.common` uses camelCase throughout; `myx.distro-*` uses PascalCase throughout — zero exceptions in either family. Never present a fact/file from one as equivalent precedent to a fact/file from the other in a report — attribute each finding to its originating project explicitly, every time it's restated, not just on first mention.
...
Reference material this skill looks specific conventions up from.
...
**Conventions this tool family actually uses**:
...
- **`project.inf`** declares `Name`, `Title`, `Requires`, `Provides`, `Declares`, `Keywords`, `Augments`, `Suggests`, `Replaces` — how projects depend on/inherit from each other, not shell `source`-ing. `Requires:`/`Provides:` express build-ordering only. `Augments` and `Suggests` are parsed and indexed and never read back by anything — `myx.distro-source/README.md:125` calls `Augments` a soft dependency hint that does not gate builds, `:126` calls `Suggests` informational only. The design consequence is the part that matters: there is no non-forking specialisation mechanism in this family, so specialising a `myx.distro-*` project means forking it or creating a new one. `Includes` and `Builders` are not keys of this schema at all.
...
**`image-prepare:sync-source-files:<sourceName>:…` reaches past the project that declares it.** `<sourceName>` takes three forms: `.` is the project's own files, `*` is this project plus the projects derived from it, and `**` is every project in the build sequence. `**` is the extension lever, and it is why severing a project is two conditions rather than one — dropping a `Requires:` line is necessary and not sufficient, because a project still in the build sequence keeps delivering its `data/` files through any `**` harvest. `myx.distro-source/README.md:161` documents the directive without any of the three forms, and the implementation lives in `myx.distro-deploy`, not `myx.distro-source`.

**Ordering offers one seam.** Every `exec-update-before` of every base runs before any `before` of the dependant, so a step cannot be inserted part-way through a base's own sequence unless that base declares the seam itself. `exec-update-after` is the only bracket, and it fires dependant-first.
...
**`DistroSourcePrepare.fn.sh`'s index-rebuild steps depend on a change-delta file that isn't always fresh.** `--scan-source-changes` populates `source-cache/new-changed.index.txt` — the same path later stages read as input — and that file dependency is real. The full documented 4-step sequence in the correct order — `--scan-source-changes` → `--sync-cached-from-source` → `--build-project-metadata` → `--ingest-distro-index-from-prepared` — still leaves `system-index/distro-provides.txt` completely untouched (same stale mtime, same stale content) even when `--scan-source-changes` correctly detects and reports a real pending change first. `--build-project-metadata` is the step that visibly breaks: it throws cascading `No such file or directory` on missing per-project `project-{declares,keywords,provides,requires}.txt` files for most projects in the tree, regardless of scan order. Whatever aggregates those per-project files into `distro-provides.txt` silently no-ops when this happens, with no top-level error. This pipeline's staleness handling does not self-heal from a plain rerun of the documented "sync + publish" command, so it is never the instrument for freshness confirmation in this workspace. Use the live no-cache query as the sole reliable verification method instead: invoke `ListDistroProjects.fn.sh` directly by full path (with `$MDLT_ORIGIN`/`$MMDAPP` exported, see above) and check its actual `--provides`/`--projects` output — that catches real problems, a genuinely broken project structure included, which the cached snapshot misses entirely.
...
**`*.fn.sh` scripts under `myx.distro-system/sh-scripts/` (e.g. `ListDistroProjects.fn.sh`) are directly executable by full path and self-bootstrap their own `$MMDAPP`/`$MDLT_ORIGIN` — no console session, no pre-exported environment, no dispatch prefix needed, for real invocations.** `$MMDAPP` is derived from the script's own path (`dirname "$0"/../../../..`) unconditionally at the top of the file; `$MDLT_ORIGIN` is set via `${MDLT_ORIGIN:=$MMDAPP/.local}` in the real-query dispatch branch. This covers `/full/path/ListDistroProjects.fn.sh --all-projects` and `--provides <tag>`-style real queries, invoked directly, nothing exported, from a plain non-console shell.

**Neither half of that generalises — read it as scoped to this package and to environment bootstrapping only.** `ListProjectSequence.fn.sh` is in `myx.distro-source/sh-scripts/`, not here, and self-bootstrapping `$MMDAPP`/`$MDLT_ORIGIN` is a different thing from resolving the same input mode: run bare it resolves `--distro-from-cached` where the console resolves `--distro-from-source`, and it under-reports with no error of any kind. Carrying the direct-invocation habit from the tools above to that one returns a quietly wrong answer, which is the worst shape a wrong answer takes. See `myx.distro-source/MAGIC.md`.

**Exception: `--help` on `ListDistroProjects.fn.sh` specifically does not self-bootstrap `$MDLT_ORIGIN`, and fails without it.** The dispatcher's `--help` branch calls the function and `exit`s immediately, before ever reaching the later code that sets `${MDLT_ORIGIN:=$MMDAPP/.local}` — so `--help` alone hits `. "$MDLT_ORIGIN/myx/.../HelpSelectProjects.include"` with `$MDLT_ORIGIN` still empty, producing a `No such file or directory` error on a visibly truncated path. This is narrow to `--help`; no real query form has this problem. Workaround if you need `--help`'s output specifically without going through a console session: export `MDLT_ORIGIN=<path-to-myx.distro-* source root>` first.
...
Each `sh-scripts/*.fn.sh` building-block command has a `Help.<Name>.include` + `Help.<Name>.help.md` pair under the owning package's `sh-lib/help/`, same "verify claims by actually running it" discipline as myx.common — but the `.help.md` file's own internal structure is myx.distro-*'s own distinct convention, not myx.common's simpler README-embedded style. Every `.help.md` in myx.distro-system takes this shape:
